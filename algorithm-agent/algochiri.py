...
## 현재 버전 현황 (2026.04.24 기준)
- 인스타그램 분석기: Ver 4.3.0
- 당근 모임 분석기: Ver 3.6.0
- 소모임×문토 분석기: Ver 2.4.0
- 카카오 오픈채팅 분석기: Ver 2.4.0
...
4. **hub.html 배경 마키** — `By Longchiri` 텍스트가 seamless 무한 루프로 흐릅니다.
   - 홀수 행(odd): 60s, 짝수 행(even): 52s 속도로 고정되어 있습니다.
   - 마키 관련 JS·CSS를 수정하지 마세요.

5. **hub.html 업데이트 예정 섹션** — 순서는 매일 → 매주 → 매월 순으로 고정입니다.
   - 현재 항목: 매일(주기적 모니터링), 매주 월(AlgoChiri 정기 점검)
   - 매월 항목은 추후 추가 예정이며, 매일·매주 아래에 붙입니다.

6. **푸터 문의 버튼** — 모든 분석기 푸터는 아래 형식으로 통일되어 있습니다:
   ```html
   <footer>
