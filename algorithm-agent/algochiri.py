...
- Reels 최대 길이 20분 (기존 90초~10분 → 20분으로 확대, 단 15~30초 짧은 Reels가 여전히 알고리즘에 유리)
- 체류 시간(Watch Time) > 조회수 우선: 저장·공유 > 댓글 > 좋아요 순서로 가중치
- 처음 5~7초 구간이 "피로 구간"으로 분류 — 빠른 Hook 필수, 인트로 없애야 함
- 비공개 DM 공유(Close Friends·DM 전달)도 알고리즘 긍정 신호로 반영
